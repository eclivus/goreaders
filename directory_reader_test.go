...
func createTestDirectory() (root string, err error) {
	...
	if err = ioutil.WriteFile(filepath.Join(root, "A.txt"), []byte("AbcAbcAbc"), 0777); err != nil {
		return
	}
	if err = os.Mkdir(filepath.Join(root, "A"), os.ModeDir|0777); err != nil {
		return
	}
	if err = ioutil.WriteFile(filepath.Join(root, "A", "B.txt"), []byte("aBcaBc"), 0777); err != nil {
		return
	}
	if err = os.Mkdir(filepath.Join(root, "A", "B1"), os.ModeDir|0777); err != nil {
		return
	}
	if err = createGzipFile(filepath.Join(root, "A", "B1", "C1.txt.gz"), "abC1abC1"); err != nil {
		return
	}
	if err = createGzipFile(filepath.Join(root, "A", "B1", "C2.txt.gz"), "abC2abC2"); err != nil {
		return
	}
	if err = os.Mkdir(filepath.Join(root, "A", "B2"), os.ModeDir|0777); err != nil {
		return
	}
	if err = createGzipFile(filepath.Join(root, "A", "B2", "C3.txt.gz"), "abC3abC3"); err != nil {
		return
	}
	return
}
